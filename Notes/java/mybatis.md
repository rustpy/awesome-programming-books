...
2. sql语句在代码中硬编码，不易维护
3. 对statement的参数设置存在硬编码问题，不易维护
4. 对结果集解析存在硬编码，不易维护

mybatis作用：

![image-20230413013433804](assets/image-20230413013433804.png)
...
2. 通过驱动管理类获取数据库连接
3. 定义sql语句
4. 预处理sql，获取statement
5. 设置sql参数
6. 向数据库发出sql并执行，获取结果集
7. 遍历结果集，封装结果对象
8. 释放资源

实例：查询user表中的所有数据

![image-20230413013916359](assets/image-20230413013916359.png)



IDEA与数据库建立链接：

![image-20230414005947019](assets/image-20230414005947019.png)



### 1、Mapper代理开发

![image-20230414010305182](assets/image-20230414010305182.png)



Mapper代理步骤;

![image-20230414012026328](assets/image-20230414012026328.png)
...
```xml
    <select id="selectAll" resultMap="popup_activity_map" parameterType="com.ctrip.flight.domain.es.model.popup.PopupActivitySearchListCondition">
        select
        <include refid="base_column_sql"/>
        from popup_activity_config
        where is_delete=0
        ...
        order by id desc
        limit #{condition.startNum}, #{condition.pageSize}
    </select>
```
...
```java
package com.ctrip.flight.domain.dal.order.dao;

import com.ctrip.flight.domain.es.model.popup.PopupActivity;
import com.ctrip.flight.domain.es.model.popup.PopupActivitySearchListCondition;
...
public interface PopupActivityMapper {
    ...
    int insert(PopupActivity popupActivity);

    int update(PopupActivity popupActivity);

    int delete(List<Long> ids);

    PopupActivity selectById(@Param(value = "id") Long id);

    //sekectall为选择所有，返回的是一个集合对象所以用list
    List<PopupActivity> selectAll(@Param(value = "condition") PopupActivitySearchListCondition condition);

    int selectCnt(@Param(value = "condition") PopupActivitySearchListCondition condition);
}
```
...
| 构件                     | 描述                                                         |
| ------------------------ | ------------------------------------------------------------ |
| SqlSessionFactoryBuilder | 根据配置信息或者代码来生成SqlSessionFactory，在构建SqlSessionFactory之前会先通过XMLConfigBuilder.parse方法解析配置文件，生成Configuration对象，然后返回一个DefaultSqlSessionFactory对象，该对象是SqlSessionFactory接口的实现类 |
| Configuration            | 核心配置类，用来存储从xml配置文件中解析的配置项              |
| MappedStatement          | 对XML文件中一个Sql语句的封装，放于Configuration对象中，通过Mapper接口的全限定名 + 方法名唯一定位一个Sql，所以Mapper中不支持方法重载 |
| Mapper                   | 屏蔽SqlSession对象，具有更强的业务可读性，该接口的方法和写sql的xml文件一一对应，该接口没有实现类，运行时，MyBatis会为这个接口生成代理类对象，代理对象会根据“接口全路径 + 方法名”去匹配xml或注解中的sql语句，然后执行并返回结果 |
| SqlSessionFactory        | 依靠工厂来创建SqlSession，存在于整个应用周期，一个数据库对应一个实例 |
| SqlSession               | 作为MyBatis的主要顶层API，完成必要的数据库增删改查功能，扮演门面的作用，真正干活的是Executor接口；即可以发送SQL去执行并返回结果，也可以获取Mapper接口；相当于JDBC中的Connection对象。它是一个线程不安全的对象 |
| ObjectFactory            | 当MyBatis在构建一个结果返回的时候，都会使用ObjectFactory去构建POJO，默认实现类是DefaultObjectFactory |
| SqlSource                | 负责根据用户传递的参数，动态地生成Sql语句，将信息封装到BoundSql对象中并返回，是MappedStatement中的一个属性 |
| Executor                 | 执行器，是MyBatis调度的核心，负责Sql语句的生成和查询缓存的维护，具体通过调度StatementHandler、ParameterHandler、ResultSetHandler来执行对应的SQL |
| StatementHandler         | 封装了JDBC Statement操作，负责对JDBC Statement的操作，如：设置参数、结果集转换 |
| ParameterHandler         | 负责对用户传递的参数转换成JDBC Statement所需要的参数         |
| ResultSetHandler         | 负责将JDBC返回的ResultSet结果集对象转换成List类型的集合      |
| TypeHandler              | 负责Java数据类型和Jdbc数据类型之间的映射和转换。可通过实现TypeHandler接口并结合@MappedTypes和@MappedJdbcTypes自定义typeHandler，然后在定义resultMap时针对指定列使用自定义的类型转换器 |
| BoundSql                 | 表示动态生成的Sql语句以及对应的参数信息，内部有3个主要参数：sql表示动态生成的sql语句；parameterObject表示参数，如果使用@Param，该属性是一个Map对象；parameterMappings是一个LIst，内部是ParameterMapping对象，保存参数的具体信息（名称、javaType、jdbcType、typeHandler等） |
...
![mybaits](assets/mybaits.png)

1. 加载配置并初始化

   将配置文件内容解析封装到Configuration中，将Sql的配置信息加载成一个MappedStatement对象，存储在内存之中。
...
3. 处理操作请求
   ...
   根据Sql的ID查找对应的MappedStatement对象；根据传入参数对象解析MappedStatement对象，得到最终要执行的Sql和参数；获取数据库连接，根据Sql和入参到数据库执行，并得到执行结果；根据MappedStatement对象中的结果映射配置对执行结果进行转换处理，得到最终的执行结果；释放连接资源。

4. 返回处理结果给用户

### 3、Mybatis插件

![image-20230416013459072](assets/image-20230416013459072.png)

装完插件后可在mapper.java文件中直接生成对应SQL语句格式

![image-20230416014234055](assets/image-20230416014234055.png)



MyBatis核心四大组件：Executor、StatementHandler、ParameterHandler、ResultSetHandler。MyBatis基于这四大组件提供了简单易用的插件扩展机制，框架支持用插件对四大核心对象进行拦截，用来增强核心对象的功能。增强功能本质上是借助底层的动态代理实现的。MyBatis中的四大对象都是代理对象。

上述四大对象的创建均是由Configuration类完成，代码如下：

```java
// org.apache.ibatis.session.Configuration 中部分代码
...
// org.apache.ibatis.plugin.InterceptorChain 中部分代码
...
从代码中可以看出，四大核心对象在创建的时候并不是直接返回的，而均是通过调用interceptorChain.pluginAll(Object)方法返回；interceptorChain中保存了所有的拦截器（插件需要实现的接口），是mybatis初始化的时候创建的。调用拦截器链中的拦截器依次对目标进行拦截或增强，interceptor.plugin(target)返回的target是被重新代理（采用JDK动态代理）后的对象，自定义插件在实现plugin方法时，会判断Object是否是自己关注的接口实现类来决定是否处理。

自定义插件需要实现Interceptor接口，通过@Intercepts和@Signature注解声明该插件所需要的拦截的方法（四大核心对象的所有方法均可被拦截），然后实现内部的3个方法：

- intercept(Invocation invocation)：插件核心方法，用于功能增强
- plugin(Object target)：生成target的代理对象，可以通过Plugin.wrap()方法生成代理对象
- setProperties(Properties properties)：传递插件所需参数
...
当实体类属性名称和数据库列表名不一致时，不能自动封装数据，此时可用：

1）**resultMap：**定义<resultMap>完全不一致的属性名和列名的映射

2）**起别名：**在SQL语句中，对不一样的列名起别名，别名和实体类属性名一样*可以定义在<sql>片段，提升复用性。

![image-20230416020940182](assets/image-20230416020940182.png)

 

**参数占位符问题：**

![image-20230416022304547](assets/image-20230416022304547.png)
